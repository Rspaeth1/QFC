...
class Trader:
    ...
    def get_strategies(self, symbols):
        ...
        take = ['RAINFOREST_RESIN', 'SQUID_INK'] #, 'KELP']
        md_adj = []
        for symbol in symbols:
            if symbol in make:
    ...
    def update_cyclical(self, cyclicals, state):
        for product in state.order_depths:
            ...
            if len(buy_orders) + len(sell_orders) != 0 and self.ticks != 0:
                ...
                #self.data[symbol] = [np.mean(self.data[product+"_RETURNS"][-30:])]
                #self.data[symbol].append(self.data[product+"_RETURNS"][-1]**2)
                dominant_freq = .4871487148714871
                sine_wave = np.sin(2*np.pi*dominant_freq*(state.timestamp/100+1))
                self.data[symbol].append(sine_wave)
       ...
       # logger.print("Observations: " + str(state.observations))
        ...
        for product in state.order_depths:
            ...
            if product in stable:
                ...
                market_take = self.enabled[product+"_TAKE"]
                if market_take and state.timestamp/100 != 0:
                    current_sine = self.data[product+"_SMA_RETURNS"][-1]
                    last_sine = self.data[product+"_SMA_RETURNS"][-2]
                    
                    if last_sine < current_sine:
                        availability = self.check_limits(state, product, side = 'BUY')
                    ...
                    z_score = (self.data[product+"_RETURNS"][-1]-ma)/std
