...
class Trader:
    ...
    def update_cyclical(self, cyclicals, state):
        ...
        try: 
            #returns difference
            #self.data['PAIRS'].append((self.data['KELP_RETURNS'][-1] - self.data['SQUID_INK_RETURNS'][-1]))
            #self.data['PAIRS'].append((self.data['SQUID_INK_RETURNS'][-1] - self.data['KELP_RETURNS'][-1]))

            
            #EMA returns difference
            period = 15
            smoothing = 2
            factor = smoothing/(period+1)
            symbol = product+"_PAIRS_EMA"
            
            #self.data[symbol].append(self.data[product+"_PAIRS"][-1]*factor+self.data[symbol][-1]*(1-factor))
        except:
            pass
    ...
    def run(self, state: TradingState):
       ...
       # logger.print("Observations: " + str(state.observations))
        ...
        ink_orders = []
        if pairs_trade:
            ma = np.mean(self.data["PAIRS"][-8:])
            std = np.std(self.data["PAIRS"][-30:])
            ...
            min_bid = min(order_depth.buy_orders.keys())
            max_ask = max(order_depth.sell_orders.keys())
            kelp_midpoint = int((min_bid + max_ask)/2)
            #kelp_price = midpoint
            
            
            
            if ma > std*(1+threshold):
                    kelp_availability = self.check_limits(state, 'KELP', side = 'SELL')
                    ink_availability = self.check_limits(state, 'SQUID_INK', side = 'BUY')
                    order_size = min(abs(kelp_availability), abs(ink_availability))/2
                    #order_size = ink_availability/2
                    
                    kelp_orders.append(Order('KELP', kelp_price, int(-order_size))) #SELL
                    ink_orders.append(Order('SQUID_INK', ink_price, int(order_size))) #BUY
                    kelp_change -= order_size
                    ink_change += order_size
            elif ma < std*(1-threshold):
                kelp_availability = self.check_limits(state, 'KELP', side = 'BUY')
                ink_availability = self.check_limits(state, 'SQUID_INK', side = 'SELL')
                order_size = min(abs(kelp_availability), abs(ink_availability))/2
                #order_size = ink_availability/2
                
                kelp_orders.append(Order('KELP', kelp_price, int(order_size))) #BUY
                ink_orders.append(Order('SQUID_INK', ink_price, int(-order_size))) #SELL
                kelp_change += order_size
                ink_change -= order_size
            else:
                kelp_pos = state.position.get('KELP',0)
                ink_pos = state.position.get('SQUID_INK',0)
                
                kelp_orders.append(Order('KELP', kelp_midpoint, int(-kelp_pos/2)))
                kelp_change += int(-kelp_pos/2)
                ink_orders.append(Order('SQUID_INK', ink_midpoint, int(-ink_pos/2)))
                ink_change += int(-ink_pos/2)
        ...
        for product in state.order_depths:
            ...
            if product == "KELP":
                total_pos_change += kelp_change
                for order in kelp_orders:
                    orders.append(order)
            elif product == "SQUID_INK":
                total_pos_change += ink_change
            ...
            if product in stable:
                ...
                market_take = self.enabled[product+"_TAKE"]
                if market_take:
                    ma = np.mean(self.data[product+"_RETURNS"])#self.data[product+"_EMA_RETURNS"][-1]###
