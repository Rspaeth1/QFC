...
class Trader:
    ...
    def get_fair_price(self, state, observation):
        window = 100
        alpha = 2/(window+1)
        ...
        X2 = -17.3008 * export_effect
        X3 = -70.1611 * import_effect
        X4 = 174.7353 * transport_effect
        X5 = 5.6057 * volume_spread_effect
        #X6 = -196.9283 * volume_imbalance_effect
        #X7 = 46.9725 * return_effect
        
        composite = X2 + X3 + X4 + X5
    ...
    def get_macaron_orders(self, state):
        ...
        if product in state.observations.conversionObservations:
            ...
            fair = self.get_fair_price(state, observation) + self.variables['macaron_start']
            panic_z, spread_z, volume_spread_z = self.get_indicators(depth, fair, midpoint)

            
            panic_threshold = 1.5
            spread_threshold = 1.5
            volume_spread_threshold = .7
            
            sunlightIndex = observation.sunlightIndex  
            
            buy_condition = panic_z < -panic_threshold or abs(volume_spread_z) > volume_spread_threshold
            sell_condition = panic_z > panic_threshold or abs(spread_z) > spread_threshold 
            buy_sunlight_mask = sunlightIndex > 55
            sell_sunlight_mask = sunlightIndex <= 45
            buy_condition = buy_condition and buy_sunlight_mask
            sell_condition = sell_condition and sell_sunlight_mask
            ...
            if state.timestamp/100 > 100:
                ...
                """
                # Conversions
                if implied_ask < best_ask + threshold and buy_condition:
                    self.conversions += 1
                elif implied_bid > best_bid - threshold and sell_condition:
                    self.conversions -= 1
                """
                
                # Clears
                if position > 0 and not (target_position < 0 or buy_condition): # Clear Longs
                    if best_bid >= fair - threshold and abs(spread_z) > spread_threshold:
                        target_position = 0
                elif position < 0 and not (target_position > 0 or sell_condition): # Clear Shorts
