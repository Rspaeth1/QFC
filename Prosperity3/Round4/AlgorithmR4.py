...
class Trader:
    ...
    def execute_spread_orders(self, state, target_pos, basket_pos, depth, basket):
        ...
        if target_pos > 0:
            ...
            order_size = min(synthetic_bid_volume, target_quantity) #min(orderbook_volume, target_quantity)# #
            ...
            order_size = min(synthetic_ask_volume, target_quantity) #min(orderbook_volume, target_quantity)# #
    ...
    def get_options_orders(self, state):
        ...
        for strike, option in options.items():
            ...
            m_t_threshold = np.clip(.3 * (1 + abs(skew)), 0.25, 1.0)# .25
    ...
    def run(self, state: TradingState):
        ...
        self.basket_params = {
            'PICNIC_BASKET1': {'spread_window': 100, 'lookback': 100, 'spread_mean': 24, 'num_std': 1, 'ema': 2, 'target_pos': 60, 'vol_multiplier': 0, 'vol_window': 45},
