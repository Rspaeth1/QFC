...
class Trader:
    ...
    def get_synthetic(self, state, target_pos, basket_pos, depth, basket):
        ...
        if target_pos > 0:            
            synthetic_bid_price = max(depth.buy_orders.keys())
            synthetic_bid_volume = abs(depth.buy_orders[synthetic_bid_price])
            
            order_size = min(synthetic_bid_volume, target_quantity)
            
            synthetic_orders = [Order('SYNTHETIC', synthetic_bid_price, -order_size)]
            return self.convert_synthetic_basket_orders(state, synthetic_orders, state.order_depths, basket)
        
        elif target_pos < 0:
            #otherwise, position wants to be lower than basket position, long synthetic and short baskets
            synthetic_ask_price = min(depth.sell_orders.keys())
            synthetic_ask_volume = abs(depth.sell_orders[synthetic_ask_price])
            
            order_size = min(synthetic_ask_volume, target_quantity)
    ...
    def spread_orders(self, state):
        ...
        if self.variables[product+'_signal'] == "SELL":
            #sell p1 buy p2
            return {**self.buy_baskets(-target_pos * ratio1, target_pos * ratio2, basket_pos_1, basket_pos_2, 'PICNIC_BASKET1', 'PICNIC_BASKET2', depth), **self.get_synthetic(state, ratio1*target_pos, basket_pos_1, synthetic_book_1, 'PICNIC_BASKET1'), **self.get_synthetic(state, -ratio2*target_pos, basket_pos_2, synthetic_book_2, 'PICNIC_BASKET2')}
        elif self.variables[product+'_signal'] == "BUY":
            #buy p1 sell p2
            return {**self.buy_baskets(target_pos * ratio1, -target_pos * ratio2, basket_pos_1, basket_pos_2, 'PICNIC_BASKET1', 'PICNIC_BASKET2', depth), **self.get_synthetic(state, -ratio1*target_pos, basket_pos_1, synthetic_book_1, 'PICNIC_BASKET1'), **self.get_synthetic(state, ratio2*target_pos, basket_pos_2, synthetic_book_2, 'PICNIC_BASKET2')}
        elif self.variables[product+'_signal'] == "CLEAR":
            return {**self.buy_baskets(0, 0, basket_pos_1, basket_pos_2, 'PICNIC_BASKET1', 'PICNIC_BASKET2', depth), **self.get_synthetic(state, 0, basket_pos_1, synthetic_book_1, 'PICNIC_BASKET1'), **self.get_synthetic(state, 0, synthetic_book_2, 'PICNIC_BASKET2')}
    ...
    def get_options_orders(self, state):
        ...
        T = ((7 - DAY) - (state.timestamp / 1000000)) / 365
        alpha = 2
        ...
        rv_series = self.realized_vol(self.data["ROCK"], alpha=alpha, window=window)
        ...
        # determine regime
        ivs_and_strikes = [(option.get('iv', np.nan), int(strike)) for strike, option in options.items()]
        ivs_and_strikes = [(iv, k) for iv, k in ivs_and_strikes if np.isfinite(iv) and iv > 0]

        if len(ivs_and_strikes) >= 3:  # Need at least 3 points for a reasonable smile fit
            ivs, strikes_filtered = zip(*ivs_and_strikes)
            coeffs = self.fit_smile_and_get_base_iv(ivs, strikes_filtered, S, T)
            a, b, c = coeffs
            prev_skew_mean = self.variables.get('skew_mean', 0)
            prev_skew_std = self.variables.get('skew_std', 0)
            self.variables['skew_mean'], self.variables['skew_std'] = self.moving_moments(prev_skew_mean, prev_skew_std, b, alpha/(window+1))
            last_skew_z = self.variables.get('skew_z', 0)
            skew_z = (b - self.variables['skew_mean'])/self.variables['skew_std']
            self.variables['skew_z'] = self.update_ema(last_skew_z, skew_z, alpha/(window+1))
            #logger.print(f"sm: {self.variables['skew_mean']}, sd: {self.variables['skew_std']}, skew: {b}, z: {self.variables['skew_z']} ")
            
            #logger.print(f"curv: {self.variables['curv']}")
        else:
            c = None  # not enough data to fit
        
        if c is not None and not np.isnan(c):
            last_c = self.variables.get('last_c', 0)
            change_in_base_iv = c - last_c
            self.variables['last_c'] = c
            self.update_boll_band(state, 'vol_history', change_in_base_iv, window=100, num_std=2, ema=2)
            self.variables['vol_regime'] = self.identify_regime(state, 'vol_history', lookback=100)
            #logger.print(f'c: {c}, volreg: {self.variables["vol_regime"]}')
            
        if len(self.data.get('vol_history', [])) > window:
            self.variables.setdefault('vol_history_signal', "CLEAR")
            self.variables['vol_history_signal'] = self.generate_signal('vol_history', self.variables['vol_regime'], 0, strict=True)
            logger.print(f"{self.variables['vol_history_signal']}")
            
        if not state.order_depths['VOLCANIC_ROCK_VOUCHER_10000'].buy_orders.keys():
            rebalance = True
            self.variables['high_vol'] = True
            logger.print("HIGH VOL")
        else:
            self.variables['high_vol'] = False
        ...
        for strike, option in options.items():
            ...
            width = 1
            #m_t_weight = np.exp(- (m_t**2) / (2 * width**2))
            #get iv/greeks
            ...
            option['gamma'] = self.bs_gamma(S, K, T, option['iv'])
            
            skew = self.variables.get('skew_mean', 0)
            m_t_threshold = np.clip(.3 * 1/(1 + abs(skew)), 0.25, 1.0)# .25
            option['ATM'] = True if abs(m_t) <= m_t_threshold else False
            #if not option['ATM']:
            #m_t_weight = 0
            
            if np.isnan(option['iv']) or np.isnan(option['vega']) or np.isnan(option['delta']) or np.isnan(option['gamma']) or np.isnan(rv) or rv is None:
                continue
            
            total_delta += option['delta']*state.position.get(product, 0)
            total_vega += option['vega']*state.position.get(product, 0)
            
            #rolling iv_vs_rv and iv_mean_revert
            prev_iv_rv_mean = option.get("iv_rv_mean", 0)
            prev_iv_rv_std = option.get("iv_rv_std", 0)
            option['iv_rv'] = option['iv'] - rv
            
            option['iv_rv_mean'], option['iv_rv_std'] = self.moving_moments(prev_iv_rv_mean, prev_iv_rv_std, option['iv_rv'], alpha/(window+1))
            #logger.print(f"rv: {rv}, iv_rv: {option.get('iv_rv_mean',0)} iv_mean_rev: {option.get('iv_mean_revert_mean')}, iv: {option.get('iv', 0)}")
            ...
            valid_trades[strike]['gamma'] = option['gamma']
        ...
        last_change_vega = self.variables.get('last_change_vega', 0)
        last_vega = self.variables.get('last_vega', 0)
        self.variables['last_vega'] = total_vega
        current_change_vega = total_vega - last_vega
        vega_change = self.update_ema(last_change_vega, current_change_vega, 5/(window+1))
        self.variables['last_change_vega'] = current_change_vega
        
        if last_change_vega > 0 and current_change_vega < 0 or last_change_vega < 0 and current_change_vega > 0:
            rebalance = True
        else:
            rebalance = False
        ...
        for strike, trade in valid_trades.items():
            if trade is None or trade.get('signal', None) is None:
                deltas.append(0)
                vegas.append(0)
            else:
        ...
        if rebalance:
            ...
            #logger.print(f'opt: {positions}, total delta: {int(total_delta)}, pos_delta: {int(position_delta)}')
            
            for index, trade in enumerate(valid_trades.values()):
                if trade.get('strike', None) is not None and trade.get('signal', None) is not None:
        ...
        for strike, option in options.items():
            ...
            prev_smoothed_pos = option.get(product+"last_pos", 0)
            smoothed_pos = self.update_ema(prev_smoothed_pos, target_pos, alpha=5/101)
            option[product+'last_pos'] = smoothed_pos 
            ...
            if state.timestamp/100 > window:
                ...
                profit = (option.get('buy_price', 0) - self.get_vwap_mid(state.order_depths[product]))*position
                if position != target_pos and abs(self.variables.get(f'last_target_pos_{product}', 0) - target_pos) > threshold:
        ...
        prev_smoothed_delta = self.variables.get("last_delta", 0)
        smoothed_delta = self.update_ema(prev_smoothed_delta, measured_delta, alpha=5/101)
        self.variables['last_delta'] = smoothed_delta 
        
        #logger.print(f"smooth: {smoothed_delta}, measured: {measured_delta}")
        
        position = state.position.get('VOLCANIC_ROCK', 0)        
        threshold = 0
        rebalance_frequency = .01
        #if state.timestamp/100 % (10000*rebalance_frequency) == 0:
        if state.timestamp/100 > window:
            target_pos = int(smoothed_delta)
            if (position != target_pos and abs(target_pos - int(prev_smoothed_delta)) >= threshold): # or (change and total_gamma > 0): 
                #logger.print(f"target_pos: {target_pos}, prev_delta: {prev_smoothed_delta}")
    ...
    def generate_volatility_signal(self, state, option, rv):
        iv_vs_rv_mean = option.get('iv_rv_mean', None)
        regime = self.variables.get('vol_regime', None)
        #logger.print(f"iv_rv_mean: {iv_vs_rv_mean}, regime: {regime}, iv_mean: {iv_mean}, mr: {iv_mean_revert_mean}")
        

        if regime is None or iv_vs_rv_mean is None:
            return None
        else:
            
            cycle = self.variables['vol_history_signal']
            

        # Generate trading signal: 1 = long vega, -1 = short vega, 0 = exit/do nothing
        signal = None
        skew_z = self.variables.get('skew_z', None)
        ...
        high_vol = self.variables['high_vol']
        if iv_vs_rv_mean > 0 and (cycle == "SELL" or high_vol): signal = -1  # Short vega (sell option)
        elif iv_vs_rv_mean < 0 and (cycle == "BUY" or high_vol): signal = 1 # Long vega (buy option)
        elif cycle == "CLEAR": signal = 0
        
        
        """
        if regime == "RIDING_UPPER":  #high vol regime
            if skew_z > skew_threshold: #high skew
                if option['ATM']:
                    signal = 1 # long atm
                else:
                    signal = -1 # short tails
            elif skew_z > -skew_threshold:
                if iv_vs_rv_mean > 0 and cycle == "SELL": signal = -1  # Short vega (sell option)
                elif iv_vs_rv_mean < 0 and cycle == "BUY": signal = 1  # Long vega (buy option)
                elif cycle == "CLEAR": signal = 0
        elif regime == "RIDING_LOWER": # low vol regime
            if skew_z < -skew_threshold:
                if option['ATM']:
                    signal = -1 # short atm
                else:
                    signal = 1 # long tails
    ...
    def moving_moments(self, prev_ema, prev_std, new_value, alpha):
        # Update EMA
        ema = alpha * new_value + (1 - alpha) * prev_ema
    
        # Update variance iteratively
        prev_variance = prev_std ** 2
        variance = (1 - alpha) * (prev_variance + alpha * (new_value - prev_ema) ** 2)
    
        return ema, variance ** 0.5
    
    def optimize_positions(
            self, d, v,
            r_bounds=(0, 200),
            dot_product_bounds=(-200, 200),
            num_samples=10000,
            batch_size=1000,
            previous_r=None,
            mutation_strength=5
        ...
        for i in range(num_samples // batch_size):
            ...
            penalty_strength = 1  # tune this
            position_change = np.sum(np.abs(r_samples - prev_r_partial), axis=1) if previous_r is not None else 0
            value_sums = (r_samples @ v_active) - penalty_strength * position_change
    ...
    def realized_vol(self, prices, alpha=2, window=100, step_size=100):
        ...
        # alpha based on smoothing window
        window = 100
        alpha = alpha / (window + 1)
    ...
    def fit_smile_and_get_base_iv(self, ivs, strikes, S, TTE):
        if S <= 0 or TTE <= 0:
            return None, None
    
        m_t = [np.log(K / S) / np.sqrt(TTE) for K in strikes]
    
        if np.any(np.isnan(ivs)) or np.any(np.isnan(m_t)):
            return None, None
    
        coeffs = np.polyfit(m_t, ivs, deg=2)  # [a, b, c]
        expected_ivs = np.polyval(coeffs, m_t)
        base_iv = coeffs[-1]  # This is c = v(m_t = 0)
    ...
    def get_fair_price_macaron(self, state, observation):
        ...
        # Regression
        X1 = 202.4845 * sunlight_effect
        X2 = -16.3726 * export_effect
        X3 = -155.7420 * import_effect
        X4 = -123.5587 * transport_effect
        X5 = -665.7801 * volume_spread_effect
        X6 = -196.9283 * volume_imbalance_effect
        X7 = 46.9725 * return_effect
    ...
    def get_macaron_orders(self, state):
        ...
        if product in state.observations.conversionObservations:
            ...
            self.variables.setdefault('macaron_start', self.get_vwap_mid(state.order_depths['MAGNIFICENT_MACARONS']))
            ...
            if state.timestamp/100 > 100:
                ...
                position = state.position.get(product, 0)
                if best_ask < fair or (position < 0 and self.variables['last_buy'] > best_ask):
                    ...
                    orders[product].extend([Order(product, int(best_ask), order_quantity)])
                    
                    self.variables['last_buy'] = best_ask
                    total_pos_change += order_quantity
                elif fair < best_bid or (position > 0 and self.variables['last_buy'] < best_bid):
                    availability = self.get_liquidity(state, product, 0, 'SELL')
                    order_quantity = min(abs(availability), best_bid_volume)
                    
                    orders[product].extend([Order(product, int(best_bid), -order_quantity)])
    ...
    def run(self, state: TradingState):
        ...
        self.basket_params = {
            'PICNIC_BASKET1': {'spread_window': 100, 'lookback': 100, 'spread_mean': 0, 'num_std': 2, 'ema': 2, 'target_pos': 60, 'vol_multiplier': 0, 'vol_window': 45},
            'PICNIC_BASKET2': {'spread_window': 50, 'lookback': 100, 'spread_mean': 0, 'num_std': 1, 'ema': 0, 'target_pos': 15},
        ...
        if 'VOLCANIC_ROCK' in state.order_depths.keys():
