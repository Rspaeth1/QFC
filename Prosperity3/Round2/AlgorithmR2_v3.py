...
class Trader:
        ...
        for order in synthetic_orders:
            ...
            if quantity > 0 and price >= best_ask:
                ...
                for component in components:
                    ...
                    if basket == 'PICNIC_BASKET1' and component == 'DJEMBES':
            ...
            for component in components:
                ...
                component_orders.setdefault(component, []).append(Order(component, component_prices[component], quantity*weight))
    ...
    def execute_spread_orders(self, target_pos, basket_pos, depth, basket):
        ...
        #logger.print(f"target_pos: {target_pos}, basket: {basket}, basket_pos: {basket_pos}, target_quantity: {target_quantity}")
    ...
    def buy_baskets(self, target_pos_1, target_pos_2, basket_pos_1, basket_pos_2, depth):
        ...
        order_size = min(abs(order_size_1), abs(order_size_2))
        
        orders = {}
        orders['PICNIC_BASKET1'] = [Order('PICNIC_BASKET1', basket_price_1, min(order_size,target_quantity_1)*sign_1)]
        orders['PICNIC_BASKET2'] = [Order('PICNIC_BASKET2', basket_price_2, min(order_size,target_quantity_1)*sign_2)]
    ...
    def spread_orders(self, state):
        ...
        logger.print(f"{basket} z-score: {z_score:.2f} | Basket Pos: {basket_pos} | Target: {target_pos}")
        """
        ...
        self.data.setdefault(product, []).append(spread_1)   
        self.data.setdefault(product, []).append(spread_2)   
        self.data.setdefault(product, []).append(spread_3)      
        if len(self.data[product]) < lookback:
            return None
        
        
        #regime = self.identify_regime(product, mean, lookback)
        
        #if regime is None:
            #return None, None
        
        #signal = self.generate_signal(product, regime, mean)
        ...
        spread_std = np.std(self.data[product])
        spread_mean = np.mean(self.data[product])
        z_score = (spread_3-spread_mean)/spread_std
        threshold = self.basket_params[basket]['num_std']
        
        self.variables.setdefault('basket_signal', None)
        if z_score > threshold:
            self.variables['basket_signal'] = "SELL"
        elif z_score < -threshold:
            self.variables['basket_signal'] = "BUY"
       
        
        
            
        logger.print(f"z-score: {z_score}, threshold: {threshold}, signal: {self.variables['basket_signal']}")

        
        if self.variables['basket_signal'] == "SELL":
            #sell p1 buy p2
            return self.buy_baskets(-target_pos, target_pos, basket_pos_1, basket_pos_2, depth)
        elif self.variables['basket_signal'] == "BUY":
            #buy p1 sell p2
            return self.buy_baskets(target_pos, -target_pos, basket_pos_1, basket_pos_2, depth)
            return orders_1, orders_2
        elif self.variables['basket_signal'] == "CLEAR":
                return self.buy_baskets(0, 0, basket_pos_1, basket_pos_2, depth)
    ...
    def run(self, state: TradingState):
        ...
        self.basket_params = {
            'PICNIC_BASKET1': {'spread_window': 50, 'lookback': 100, 'spread_mean': 48.64155846485088, 'num_std': 2, 'target_pos': 15},
            'PICNIC_BASKET2': {'spread_window': 50, 'lookback': 100, 'spread_mean': 31.792656681639464, 'num_std': 2, 'target_pos': 15},
            'SYNTHETIC_3': {'spread_window': 50, 'lookback': 100, 'spread_mean': 0, 'num_std': 1.5, 'target_pos': 60}
        }
        ...
        orders = self.spread_orders(state)
