...
class Trader:
    ...
    def get_options_orders(self, state):
        ...
        if len(ivs_and_strikes) >= 3:  # Need at least 3 points for a reasonable smile fit
            ...
            a, b, c = coeffs
            self.variables['skew'] = b
            #logger.print(f"curv: {self.variables['curv']}")
        else:
            c = None  # not enough data to fit
        
        if c is not None:
            self.update_boll_band(state, 'vol_history', c, window=100, num_std=1.5, ema=2, vol_multiplier=0, vol_window=30)
        ...
        for strike, option in options.items():
            ...
            skew = self.variables.get('skew', 0)  # fit from smile
            m_t_threshold = np.clip(0.1 * rv / option['iv'] * (1 + abs(skew)), 0.25, 1.0)
            option['ATM'] = True if abs(m_t) <= m_t_threshold else False
            #if not option['ATM']:
            #m_t_weight = 0
            
            if np.isnan(option['iv']) or np.isnan(option['vega']) or np.isnan(option['delta'] or np.isnan(rv)):
                continue
       ...
       # if state.timestamp/100 % 100 == 0:
        ...
        #self.variables['last_gamma'] = total_gamma
        
        return orders
    
    def generate_volatility_signal(self, state, iv_mean, iv_mean_revert, iv_vs_rv, weight_mean_reversion=0.5, threshold=0.02):
        regime = self.variables.get('vol_regime', None)
        if regime is None:
            return None
        else:
            self.variables.setdefault('vol_history_signal', "CLEAR")
            self.variables['vol_history_signal'] = self.generate_signal('vol_history', regime, iv_mean, strict=False)
        ...
        signal = None
        #score *= m_t_weight
        if iv_vs_rv > 0 and cycle == "SELL": signal = -1  # Short vega (sell option)
        elif iv_vs_rv < 0 and cycle == "BUY": signal = 1  # Long vega (buy option)
