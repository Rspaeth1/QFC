...
class Trader:
    ...
    def get_options_orders(self, state):
        ...
        if len(ivs_and_strikes) >= 3:  # Need at least 3 points for a reasonable smile fit
            ...
            a, b, c = coeffs
            self.variables['curv'] = a
            #logger.print(f'ivs: {ivs}, c: {c}')
        else:
            c = None  # not enough data to fit
        
        if c is not None:
            self.update_boll_band(state, 'vol_history', rv-c, window=100, num_std=1.5,ema=0, vol_multiplier=0, vol_window=30)
        ...
        for strike, option in options.items():
            ...
            m_t_threshold =  .3
            width = 1
            m_t_weight = m_t_threshold + np.exp(- (m_t**2) / (2 * width**2))
            
            #get iv/greeks
            option['iv'] = self.implied_vol_newton(C_market, S, K, T)
            d1 = (np.log(S / K) + 0.5 * option['iv']**2 * T) / (option['iv'] * np.sqrt(T))
            option['vega'] = self.bs_vega(S, K, T, d1)
            option['delta'] = self.bs_delta(S, K, T, d1)
            option['gamma'] = self.bs_gamma(S, K, T, option['iv'], d1)
            option['ATM'] = True if abs(m_t) <= m_t_threshold else False
            #if not option['ATM']:
            #m_t_weight = 0
            
            if np.isnan(option['iv']) or np.isnan(option['vega']) or np.isnan(option['delta'] or np.isnan(rv) or np.isnan(option['gamma'])):
                continue
            
            total_delta += option['delta']*state.position.get(product, 0)
            
            #rolling iv_vs_rv and iv_mean_revert
            option['iv_mean'] = self.get_expected_iv(K, S, T)
            prev_iv_rv = option.get("iv_rv", 0)
            prev_iv_mean_rev = option.get("iv_mean_revert", 0)
            iv_rv = option.get('iv', 0) - rv
            iv_mean_rev = option.get('iv', 0) - option['iv_mean']
            #logger.print(f"rv: {rv}, iv_rv: {iv_rv} iv_mean_rev: {iv_mean_rev}, iv: {option.get('iv', 0)}")
            ...
            signal = self.generate_volatility_signal(
                m_t_weight=m_t_weight,
                iv_mean=option['iv_mean'],
                iv_mean_revert=option['iv_mean_revert'],
                iv_vs_rv=option['iv_rv'],
                weight_mean_reversion=1,
                threshold=.02 #*rv/option['iv']
            )
        ...
        # optimize gamma/delta, make best trade based on gamma/delta
        deltas = []
        gammas = []
        ...
        for strike, trade in valid_trades.items():
            if trade is None or trade.get('signal', None) is None:
                ...
                deltas.append(trade.get('delta', 0))
                gammas.append(trade.get('gamma', 0))
            
        
       # if state.timestamp/100 % 100 == 0:
        positions, position_delta, position_gamma = self.optimize_positions(deltas, gammas,  previous_r=self.variables.get('prev_r', [0, 0, 0, 0, 0]))
        ...
        for index, trade in enumerate(valid_trades.values()):
            if trade.get('strike', None) is not None and trade.get('signal', None) is not None:
                options[trade['strike']]['target_pos'] = positions[index]
                #logger.print(f'target_pos: {positions[index]}, signal: {trade["signal"]}')
            
        
        # update positions
        threshold = 0
        #switch = True if state.timestamp/100 % 50 == 0 else False
        for strike, option in options.items():
            product = "VOLCANIC_ROCK_VOUCHER_"+str(strike)
            position = state.position.get(product, 0)
        ...
        smoothed_delta = self.update_ema(prev_smoothed_delta, measured_delta, alpha=2/101)
        ...
        if state.timestamp/100 > window:
            ...
            if position != target_pos and abs(target_pos - int(prev_smoothed_delta)) >= threshold: 
    ...
    def generate_volatility_signal(self, m_t_weight, iv_mean, iv_mean_revert, iv_vs_rv, weight_mean_reversion=0.5, threshold=0.02):
        regime = self.variables.get('vol_regime', None)
        if regime is None:
            return None
        else:
            self.variables.setdefault('vol_history_signal', "CLEAR")
            self.variables['vol_history_signal'] = self.generate_signal('vol_history', regime, iv_mean, strict=True)
        ...
        #logger.print(f"weight_mean_reversion: {weight_mean_reversion}, score: {score}, iv_rv: {iv_vs_rv} iv_mean_rev: {iv_mean_revert}")

        # Generate trading signal: 1 = long vega, -1 = short vega, 0 = exit/do nothing
        inner_threshold = .01
        signal = None
        score *= m_t_weight
        if cycle == "BUY": signal = 1  # buy
        elif cycle == "SELL": signal = -1  # sell
    ...
    def optimize_positions(
            ...
            previous_r=None,
            mutation_strength=5,
            lambda_gamma=1.0  # Penalty weight for gamma exposure
            ):
        d = np.array(deltas)
        g = np.array(gammas)
        assert len(d) == len(g), "deltas and gammas must be the same length"
    
        num_vars = len(d)
        full_r = np.zeros(num_vars, dtype=int)
        mask_nonzero = (d != 0) | (g != 0)
        active_indices = np.where(mask_nonzero)[0]
        d_active = d[mask_nonzero]
        g_active = g[mask_nonzero]
        dim = len(d_active)
        ...
        for i in range(num_samples // batch_size):
            ...
            dot_deltas = r_samples @ d_active
            dot_gammas = r_samples @ g_active
    
            delta_penalty = np.abs(dot_deltas)
            gamma_penalty = np.abs(dot_gammas)
            score = - (delta_penalty + lambda_gamma * gamma_penalty)
    
            max_idx = np.argmax(score)
            if score[max_idx] > best_value:
                best_value = score[max_idx]
                best_r_partial = r_samples[max_idx]
                best_delta = dot_deltas[max_idx]
                best_gamma = dot_gammas[max_idx]
    
        if best_r_partial is not None:
            full_r[active_indices] = best_r_partial
    
        return full_r, best_delta, best_gamma
    ...
    def get_expected_iv(self, strike, S, TTE):
        smile_coeffs = [0.22032054, 0.00876318, 0.15727743]
        
        if S <= 0 or TTE <= 0:
            return np.nan

        m_t = np.log(strike / S) / np.sqrt(TTE)
        expected_iv = np.polyval(smile_coeffs, m_t)
        return expected_iv
    ...
    def bs_call_price(self, S, K, T, sigma, d1):
        if sigma <= 0 or T <= 0:
            return max(S - K, 0)
    ...
    def bs_vega(self, S, K, T, d1):
        if T <= 0:
            return 0
        return S * self.norm_pdf(d1) * np.sqrt(T)
    
    def bs_delta(self, S, K, T, d1):
        if T <= 0:
            return 0.0
        return self.norm_cdf(d1)  # For a call option
    
    def bs_gamma(self, S, K, T, sigma, d1):
    ...
    def implied_vol_newton(self, C_market, S, K, T, sigma_init=0.15, tol=1e-5, max_iter=100):
        ...
        for i in range(max_iter):
            ...
            price = self.bs_call_price(S, K, T, sigma, d1)
            vega = self.bs_vega(S, K, T, d1)
