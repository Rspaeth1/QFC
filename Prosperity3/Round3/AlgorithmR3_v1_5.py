...
class Trader:
    ...
    def generate_signal(self, product, regime, true_mean, strict=False, custom=False, custom_product=0, custom_last=0):
        ...
        if custom:
            price = custom_product
            last_price = custom_last
        else:
            price = self.data[product][-1]
    ...
    def get_options_orders(self, state):
        ...
        orders = {}
        options = {
            9500: {'iv_mean': .2463, 'weight': .5},
            9750: {'iv_mean': .1968, 'weight': .5},
            10000: {'iv_mean': .1709, 'weight': .25},
            10250: {'iv_mean': .1600, 'weight': .25},
            10500: {'iv_mean': .1632, 'weight': .25}
        }
        ...
        for strike, option in options.items():
            ...
            #signal = self.generate_volatility_signal(option['iv'], rv, option['iv_mean'], weight_mean_reversion = option['weight'], threshold=.1)
            if self.update_boll_band(state, product, option['iv'], window, num_std=1.5) is None:
                continue
            
            regime = self.identify_regime(state, product, lookback=100)
            
            if regime is None:
                continue
            
            self.variables.setdefault(product+'_signal', None)
            self.variables[product+'_signal'] = self.generate_signal(product, regime, option['iv_mean'], strict=False, custom=True, custom_product=rv, custom_last=self.variables['last_rv'])
            ...
            signal = None
            sig = self.variables[product+"_signal"]
            if sig == "BUY":
                signal = 1
            elif sig == "SELL":
                signal = -1
        ...
        positions, total_delta = self.optimize_positions(deltas, vegas, previous_r=self.variables.get('prev_r', [0, 0, 0, 0, 0]))
        ...
        self.variables['last_total_delta'] = -total_delta  
        self.variables['last_rv'] = rv
    ...
    def generate_volatility_signal(self, iv, rv, iv_mean, weight_mean_reversion=0.5, threshold=0.05):
        if any(np.isnan(x) for x in [iv, rv, iv_mean]) or not np.isfinite(weight_mean_reversion):
            return 0  # or np.nan, or skip
        ...
        #logger.print(f"score: {score}, iv_mr {iv_mean_revert}, ivrv {iv_vs_rv}")
