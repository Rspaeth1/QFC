...
class Trader:
    ...
    def get_options_orders(self, state):
        ...
        weight_map = {
            "low": 1,     # strong mean-reversion
            "neutral": 1,
            "high": 1  # less trust in mean reversion
        }
        ...
        ivs = [option.get('iv', 0) for option in options.values()]
        _, c = self.fit_smile_and_get_base_iv(ivs, strikes, S, T)
        self.data.setdefault('vol_history', [])
        if c is not None:
            self.data['vol_history'].append(c)
        self.variables.setdefault('option_regime', 'neutral')
        regime = self.variables['option_regime']
        alpha = alpha_map[regime]
        
        if state.timestamp/100 % 10 == 0:
            self.variables['option_regime'] = self.get_option_regime()
        ...
        for strike, option in options.items():
           ...
           # m_t_threshold = .5
            width = m_t_map[regime] #2*option.get('iv', 0)
            ...
            signal = self.generate_volatility_signal(
                ...
                weight_mean_reversion=weight_map[regime],
       ...
       # if state.timestamp/100 % 100 == 0:
        positions, position_delta = self.optimize_positions(deltas, vegas, dot_product_bounds=(-hedge_map[regime], hedge_map[regime]),  previous_r=self.variables.get('prev_r', [0, 0, 0, 0, 0]))
        ...
        for strike, option in options.items():
            ...
            if (position != target_pos and abs(self.variables.get(f'last_target_pos_{product}', 0) - target_pos) > threshold and not change) or (change and profit >= 0):
        ...
        smoothed_delta = self.update_ema(prev_smoothed_delta, measured_delta, alpha=2*alpha/101)
    ...
    def generate_volatility_signal(self, m_t_weight, iv_mean_revert, iv_vs_rv, weight_mean_reversion=0.5, threshold=0.05):
    ...
    def get_option_regime(self, window=100):
        vol_history = self.data['vol_history']
        if len(vol_history) >= window:
            rolling_c = np.array(vol_history[-window:])
            vol_mean = np.mean(rolling_c)
            vol_std = np.std(rolling_c)
        
            current_c = self.data['vol_history'][-1]
            num_std = 1.5
        
            if current_c > vol_mean + num_std * vol_std:
                regime = 'high'
            elif current_c < vol_mean - num_std * vol_std:
                regime = 'low'
            else:
                regime = 'neutral'
        else:
