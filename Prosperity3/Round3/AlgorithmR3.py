...
class Trader:
    ...
    def get_options_orders(self, state):
        ...
        orders = {}
        options = {
            9500: {'iv_mean': .1687, 'hedge': 0},
            9750: {'iv_mean': .1860, 'hedge': 0},
            10000: {'iv_mean': .1709, 'hedge': 0},
            10250: {'iv_mean': .1600, 'hedge': 0},
            10500: {'iv_mean': .1632, 'hedge': 0},
        }
        ...
        for strike, option in options.items():
            ...
            #update log returns, iv, ema
            option.setdefault('prev_fair', 0)
            last_price = option['prev_fair']
            if last_price == 0:
                option['prev_fair'] = C_market
                continue
            
            log_return = self.get_log_return(C_market, last_price)
            
            option.setdefault('prev_return', 0)
            last_return = option['prev_return']
            if last_return == 0:
                option['prev_return'] = log_return
                continue
            
            window = 100
            alpha = 2/(window+1)
            option['rv'] = self.get_realized_vol(last_return, log_return, alpha)
            option.setdefault('ema', C_market)
            option['ema'] = self.update_ema(option['ema'], C_market, alpha)
            ...
            signal = self.generate_volatility_signal(option['ema'], option['iv'], option['rv'], option['iv_mean'], weight_mean_reversion = 1)
            #logger.print(f'OSig: {signal}, Strike: {strike}')
            
            if signal != 0:
                v_d_ratio = option['vega']/abs(option['delta'])
                valid_trades.setdefault(v_d_ratio,[]).extend([{'product': product, 'delta': option['delta'], 'vega': option['vega'], 'signal': signal, 'strike': strike}])
            
            # if open position, check if we should exit
            position = state.position.get(product, 0)
            if position != 0:
                ...
                if signal != sign:
                   ...
                   price = best_bid if signal < 0 else best_ask
                   volume = best_bid_volume if signal < 0 else best_ask_volume
                   
                   if price == 0 or volume == 0:
                       continue
                   
                   order_size = min(abs(volume), abs(position))*-sign
                    
                   option['target_pos'] = 0
                   orders[product] = [Order(product, price, order_size)]
                   self.variables['total_hedge'] -= option['hedge']
                   logger.print(f"h {option['hedge']}")
                   

                elif position != option.get('pos_target', 0):
                    depth = state.order_depths[product]
                    best_bid = max(depth.buy_orders.keys()) if depth.buy_orders else 0
                    best_ask = min(depth.sell_orders.keys()) if depth.sell_orders else 0
                    best_bid_volume = depth.buy_orders[best_bid] if depth.buy_orders else 0
                    ...
                    sign = 1 if option['pos_target'] - position > 0 else -1
        ...
        if not valid_trades:
            ...
            logger.print(self.variables['options_data'])
            return None
        
        
        # sort by highest, make best trade based on vega/delta
        
        sorted_keys = sorted(valid_trades.keys(), reverse=True)
        for key in sorted_keys:
            
            for trade in valid_trades[key]:
                ...
                availability = self.get_liquidity(state, product, 0, signal)
                delta_direction = 1 if delta > 0 else -1
                volcanic_rock_available = self.get_liquidity(state, 'VOLCANIC_ROCK', volc_change, -delta_direction, self.variables.get('total_hedge', 0))
                
                if abs(delta) < 1e-6:
                    max_contracts = 0
                else:
                    max_contracts = min(abs(volcanic_rock_available / abs(delta)), self.limits[product])

                
                order_size = min(abs(volume), abs(availability), abs(int(max_contracts)))*signal
                hedge_size = int(max_contracts*-delta)
                

                

                options[trade['strike']]['target_pos'] = max_contracts
                options[trade['strike']]['hedge'] += hedge_size
                options[trade['strike']]['buy_price'] = price
                self.variables.setdefault('total_hedge', 0)
                self.variables['total_hedge'] += hedge_size
                volc_change += hedge_size
            
                orders[product] = [Order(product, price, order_size)]
                
                
        # ensure volcanic_rock stays in line with options for hedge
        
        position = state.position.get('VOLCANIC_ROCK', 0)
        total_hedge = self.variables.get('total_hedge', 0)
        logger.print(f'Total hedge: {total_hedge}')
        
        if position != total_hedge: 
            ...
            target_pos = total_hedge - position
            sign = 1 if target_pos > 0 else -1
            ...
            if price != 0 and volume != 0:
                ...
                availability = self.get_liquidity(state, 'VOLCANIC_ROCK', 0, sign)
                order_size = min(abs(target_pos), abs(volume), abs(availability))*sign
    
            
                orders['VOLCANIC_ROCK'] = [Order('VOLCANIC_ROCK', price, order_size)]
    ...
    def get_log_return(self, price_now, price_prev):
        if price_prev < 1e-6:
            ...
            return np.log(price_now / price_prev)

    def generate_volatility_signal(self, ema, iv, rv, iv_mean, weight_mean_reversion=0.5, threshold=0.02):

        # Calculate sub-signals
        iv_mean_revert = iv - iv_mean
        iv_vs_rv = iv - rv
    ...
    def get_realized_vol(self, log_return, alpha, scale=1):
        if log_return < 1e-6:
            return 0
        else:
            realized_vol = np.sqrt(log_return) * np.sqrt(scale)
            return realized_vol
